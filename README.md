# 基于多模态人工智能感知困境儿童心理健康动态评估系统 
本发明涉及人工智能技术领域，特别是涉及基于多模态人工智能感知困境儿童心理健康动态评估系统，包括多模态数据采集模块、数据预处理模块、特征提取与困境识别模块以及报告生成与输出模块，系统通过采集困境儿童的语音、微表情、文本、心理测验和行为轨迹五类数据，对原始数据进行预处理后，通过多模态特征融合建立特征与困境类型的映射关系，精准识别困境儿童所属的困境类型与风险等级，并生成结构化心理健康评估报告，实现对困境儿童心理状态的动态追踪与长期监测，本发明还可包括干预方案生成模块，为各类困境儿童匹配差异化干预方案，通过多模态数据融合、动态监测机制和个性化干预建议，本系统将困境类型识别准确率从70%提升至95%以上。
